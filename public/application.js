...
/***/ (function(module, exports, __webpack_require__) {
	...
	var setAPIOrigin = __webpack_require__(38);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var changePasswordTemplate = __webpack_require__(7);
	var getFormFields = __webpack_require__(27);
	var objGameEvents = __webpack_require__(29);
	var Player = __webpack_require__(35);
	var playerTemplate = __webpack_require__(36);
	var signInRegisterTemplate = __webpack_require__(37);
	...
	var onChange = function onChange(e) {
	  // ALWAYS preventDefault first!
	  e.preventDefault();
	  authAPI.changePassword(getFormFields(e.target)).then(changePasswordSuccess).catch(changePasswordFailure);
	};
	...
	var changePasswordResults = function changePasswordResults(isChanged) {
	  // Remove click handlers
	  ...
	  var message = isChanged ? store.objPlayer.name + '\'s password was changed.' : 'Could not change' + store.objPlayer.name + '\'s password right now. Try again later.';
	...
	// ===== ===== ===== ===== ===== ===== ===== ===== ===== =====
	// Log-in functions
	var addLogInRegister = function addLogInRegister() {
	...
	var displayLoggedInPlayer = function displayLoggedInPlayer(message) {
	...
	var onLogIn = function onLogIn(e) {
	  e.preventDefault();
	  var objProfferedCredentials = getFormFields(e.target);
	  // If there are no cached credentials, create them
	  if (!store.objPlayer) {
	    var objPlayer = new Player(null, // not logged in yet
	    objProfferedCredentials.credentials.email, null, // no ID
	    null, // no token
	  ...
	  authAPI.signIn(objProfferedCredentials).then(logInSuccess).catch(logInFailure);
	};

	var logInSuccess = function logInSuccess(objResponse) {
	  ...
	  objResponse.user.email, objResponse.user.id, objResponse.user.token);
	...
	var logInFailure = function logInFailure(objResponse) {
	...
	var onLogOut = function onLogOut(e) {
	  ...
	  authAPI.signOut().then(logOutSuccess).catch(logOutFailure);
	};

	var logOutSuccess = function logOutSuccess(objResponse) {
	  // Remove 'logging out' message and display log-in/register forms
	  $('#announcement').html('');
	  addLogInRegister();
	};

	var logOutFailure = function logOutFailure(objResponse) {
	...
	var onClickClearAnnouncement = function onClickClearAnnouncement() {
	  $('#announcement').html('');
	};
	...
	var onSignUp = function onSignUp(e) {
	  ...
	  if (/\s/.test(store.objPlayer.name.slice(-1)) || /\s/.test(store.objPlayer.name.slice(0))) {
	...
	var signUpSuccess = function signUpSuccess(objResponse) {
	  // Display welcome announcement.
	  $('#announcement').html('Welcome, ' + objResponse.user.email + '. Logging in…');
	  var objCredentials = {
	    credentials: {
	  ...
	  authAPI.signIn(objCredentials).then(logInSuccess).catch(logInFailure);
	};

	var signUpFailure = function signUpFailure(objResponse) {
	  $('#announcement').html('That name is probably already taken. Try something else.');
	};

	module.exports = { addLogInRegister: addLogInRegister };
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var changePassword = function changePassword(objProfferedCredentials) {
	  return $.ajax({
	...
	var signIn = function signIn(objProfferedCredentials) {
	...
	var signUp = function signUp(objProfferedCredentials) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	    return "<!-- Password change HTML -->\n<form id=\"change-password\">\n  <fieldset>\n    <input type='password'\n    name='credentials[old]' placeholder='old password'>\n\n    <input type='password'\n    name='credentials[new]' placeholder='new password'>\n\n    <input type=\"submit\" name=\"submit\" value=\"Change\">\n  </fieldset>\n</form>\n<input type=\"button\" id=\"cancel-button\" value=\"Cancel\">\n";
	},"useData":true});
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var addNestedValue = __webpack_require__(28);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var gameAPI = __webpack_require__(33);
	var gameGridTemplate = __webpack_require__(34);
	var store = __webpack_require__(6);
	...
	var newGame = function newGame() {
	  ...
	  if (!store.objPlayer.suppressStatistics) {
	    ...
	    gameAPI.getMyGames().then(getMyGamesSuccess).catch(getMyGamesFailure);
	  }
	  ...
	  // Load new game grid
	  var gameGridHtml = gameGridTemplate();
	...
	var getMyGamesSuccess = function getMyGamesSuccess(objResponse) {
	...
	var getMyGamesFailure = function getMyGamesFailure(objResponse) {
	...
	var onPlayAgain = function onPlayAgain(e) {
	  // ALWAYS preventDefault first!
	  e.preventDefault();
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var AllPaths = __webpack_require__(31);
	var gameAPI = __webpack_require__(33);
	var store = __webpack_require__(6);
	...
	Game.prototype.addMove = function (intSquareIndex) {
	  ...
	  var objAllPaths = new AllPaths(this.arrSquareStates);
	...
	var addMoveToServer = function addMoveToServer(objGame, intSquareIndex) {
	  // Update server, if a game ID was created earlier.
	  if (objGame.id) {
	    gameAPI.addMove(intSquareIndex, objGame.strTurn).then(onAddMoveSuccess).catch(onAddMoveFailure);
	  } else {
	...
	var onAddMoveSuccess = function onAddMoveSuccess(objResponse) {
	...
	var onAddMoveFailure = function onAddMoveFailure(objResponse) {
	  ...
	  if (store.objGame.serverAttempts++ < 6) {
	    ...
	    // verification.
	    gameAPI.addMove(4, store.objGame.arrSquareStates[4]).then(onAddMoveSuccess).catch(onAddMoveFailure);
	  }
	};

	// ===== ===== ===== =====
	// Functions to start a game on the server
	var startGame = function startGame(objGame) {
	  ...
	  if (objGame.serverAttempts++ < 6) {
	    gameAPI.startGame().then(onStartGameSuccess).catch(onStartGameFailure); // Retry up to 6 times.
	  }
	};

	var onStartGameSuccess = function onStartGameSuccess(objResponse) {
	  store.objGame.serverAttempts = 0; // Reset server attempt counter
	...
	var onStartGameFailure = function onStartGameFailure(objResponse) {
	  // Try to start the game on the server again, if less than 6 attempts made to date.
	  startGame(store.objGame);
	};

	var verifyGame = function verifyGame(objResponse) {
	  ...
	  for (var i = 0; i < 9; i++) {
	    if (objResponse.game.cells[i] !== store.objGame.arrSquareStates[i]) {
	      gameAPI.addMove(i, store.objGame.arrSquareStates[i]).then(onAddMoveSuccess).catch(onAddMoveFailure);
	      return;
	    }
	  }
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var AllPaths = function AllPaths(arrSquareStates) {
	  this._arrAllPaths = [
	  // create array of all paths & set their state according to current game state
	  new Path([0, 1, 2], arrSquareStates), // rows
	  new Path([3, 4, 5], arrSquareStates), new Path([6, 7, 8], arrSquareStates), new Path([0, 3, 6], arrSquareStates), // columns
	  new Path([1, 4, 7], arrSquareStates), new Path([2, 5, 8], arrSquareStates), new Path([0, 4, 8], arrSquareStates), // diagonals
	  ...
	  var _arrWinningPaths = [];
	  var _strWinningMark = '';
	  var _intFullPathCount = 0;
	  // Check every path
	  for (var j = 0; j < 8; j++) {
...
/***/ (function(module, exports) {
	...
	var Path = function Path(arrIndices, arrSquareStates) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var addMove = function addMove(intCell, strXO) {
	  return $.ajax({
...
/***/ (function(module, exports) {
	...
	var Player = function Player(bool, strName, strId, strAuthNToken, strPassword) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	function __default(obj) { return obj && (obj.__esModule ? obj["default"] : obj); }
	module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	    return "<!-- Player while in game -->\n<div class=\"row\">\n\n  <div id=\"player-name\" class=\"player-name col-xs-offset-3 col-xs-4\">\n  </div>\n\n  <div id=\"player-actions\" class=\"player-actions col-xs-2\">\n  </div>\n</div>\n";
	},"useData":true});
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var parseNestedQuery = __webpack_require__(39);
